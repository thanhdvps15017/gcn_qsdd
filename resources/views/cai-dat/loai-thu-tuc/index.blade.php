                                    ...
                                    <td class="text-end position-static">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end shadow border-0 rounded-3">
                                                ...
                                                <form action="{{ route('settings.loai-thu-tuc.destroy', $item) }}" method="POST"
                                                    onsubmit="confirmDelete(event, this, 'Xoá thủ tục «{{ addslashes($item->name) }}» ?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                    ...
                    <div class="modal-body row g-3 p-4">
                        <div class="col-12">
                            ...
                            <input type="text" name="name" id="nameInput" class="form-control"
                                value="{{ old('name') }}" placeholder="Nhập tên loại thủ tục">
                            @error('name')
                        ...
                        <div class="col-12">
                            ...
                            <input type="number" name="processing_days" id="daysInput" class="form-control"
                                value="{{ old('processing_days') }}" min="1" max="365" placeholder="Nhập số ngày (VD: 5)">
                        </div>
                    </div>
...
@push('script')
    <script>
        ...
        function openAddModal() {
            form.reset();
            form.action = "{{ route('settings.loai-thu-tuc.store') }}";
        ...
        document.addEventListener('DOMContentLoaded', () => {
            @if ($errors->any())
                @if (old('_method') == 'PUT')
                    form.action = `/settings/loai-thu-tuc/{{ old('id') }}`;
                    document.getElementById('modalTitle').innerText = 'Sửa loại thủ tục';
                    document.getElementById('methodField').disabled = false;
                @else
                    form.action = "{{ route('settings.loai-thu-tuc.store') }}";
